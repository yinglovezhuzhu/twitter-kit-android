...
public class TweetUploadService extends IntentService {
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        uploadTweet(taskId, twitterSession, tweetText, imageUri);
    }

    void uploadTweet(final String taskId, final TwitterSession session, final String text, final Uri imageUri) {
        if (imageUri != null) {
            uploadMedia(taskId, session, imageUri, new Callback<Media>() {
                @Override
                public void success(Result<Media> result) {
                    uploadTweetWithMedia(taskId, session, text, result.data.mediaIdString);
                }
            ...
            uploadTweetWithMedia(taskId, session, text, null);
        }
    }
                ...
                .enqueue(
                        new Callback<Tweet>() {
                            ...
                            public void success(Result<Tweet> result) {
                                sendSuccessBroadcast(taskId, result.data.getId());
                                stopSelf();
                            }
    ...
    void uploadMedia(final String taskId, TwitterSession session, Uri imageUri, Callback<Media> callback) {
        final TwitterApiClient client = dependencyProvider.getTwitterApiClient(session);

        final String path = FileUtils.getPath(TweetUploadService.this, imageUri);
        if (path == null) {
            fail(taskId, new TwitterException("Uri file path resolved to null"));
            return;
        }
    ...
    void fail(final String taskId, TwitterException e) {
        sendFailureBroadcast(taskId, intent);
        Twitter.getLogger().e(TAG, "Post Tweet failed", e);
        stopSelf();
    }

    void sendSuccessBroadcast(final String taskId, long tweetId) {
    ...
    void sendFailureBroadcast(final String taskId, Intent original) {
